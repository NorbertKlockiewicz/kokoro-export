"""
Test generator upsampling stages one by one
Find which specific stage produces NaNs
"""
...
print("Testing Generator Upsampling Stages")
print("=" * 80)
...
# Generator has 5 upsampling stages (ups) with resblocks
# Let's test each stage progressively

for stage in range(gen.num_upsamples + 1):  # +1 to include final conv_post
    print("=" * 80)
    if stage < gen.num_upsamples:
        print(f"TEST: Upsampling stages 0-{stage} (of {gen.num_upsamples-1})")
    else:
        print(f"TEST: All upsampling + conv_post")
    print("=" * 80)

    class GenStages(nn.Module):
        def __init__(self, gen, num_stages):
            super().__init__()
            self.gen = gen
        ...
        def forward(self, x, s, har):
            ...
            # If all stages done, add conv_post
            if self.num_stages == self.gen.num_upsamples:
                x = torch.nn.functional.leaky_relu(x)
                x = self.gen.conv_post(x)

            return x

    test_stages = GenStages(gen, stage if stage < gen.num_upsamples else gen.num_upsamples).eval()

    with torch.no_grad():
        output_pt = test_stages(x_dec, ref_s, har)
        print(f"PyTorch: shape={output_pt.shape}, range=[{output_pt.min():.3f}, {output_pt.max():.3f}], NaN={output_pt.isnan().any()}")

        exported = export(test_stages, (x_dec, ref_s, har), strict=False)
        output_exp = exported.module()(x_dec, ref_s, har)
        ...
        try:
            ...
            pte_path = f"exported_pte/debug/gen_stage_{stage}.pte"
            with open(pte_path, "wb") as f:
                f.write(edge_program.buffer)

            program = runtime.load_program(pte_path)
            method = program.load_method("forward")
            outputs = method.execute((x_dec, ref_s, har))
            output_et = outputs[0]
            ...
            if output_et.isnan().any():
                ...
                print(f"⚠ Stage {stage} FAILS with {nan_pct:.2f}% NaNs")
                print(f"   --> NaNs first appear at stage {stage}")
                break
            else:
...
print("\n" + "=" * 80)
print("The first stage that produces NaNs is the culprit")
print("=" * 80)
