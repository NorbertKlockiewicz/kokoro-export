"""
Test encode block only - the input is already concatenated
"""
...
with torch.no_grad():
    ...
    # Pre-compute concatenated input
...
# Test: encode block with pre-concatenated input
...
with torch.no_grad():
    output_pt = encode_only(x_cat, ref_s)
    print(f"PyTorch: range=[{output_pt.min():.3f}, {output_pt.max():.3f}]")

    exported = export(encode_only, (x_cat, ref_s), strict=False)
    edge_program = to_edge_transform_and_lower(
        exported, partitioner=[XnnpackPartitioner()]
    ).to_executorch()

    pte_path = "exported_pte/debug/encode_only.pte"
    with open(pte_path, "wb") as f:
        f.write(edge_program.buffer)

    program = runtime.load_program(pte_path)
    method = program.load_method("forward")
    outputs = method.execute((x_cat, ref_s))
    output_et = outputs[0]
    print(output_et)
    has_nan = output_et.isnan().any()
    print(
        f"ExecuTorch: range=[{output_et.min():.3f}, {output_et.max():.3f}], NaN={has_nan}"
    )
