"""
Test Encode block with XNNPACK partitioner
"""
...
print("Testing Encode with XNNPACK")
print("=" * 80)
...
# Test Encode block with XNNPACK
class EncodeTest(nn.Module):
...
encode_test = EncodeTest(model.decoder).eval()

with torch.no_grad():
    x_enc = encode_test(asr, F0_pred, N_pred, ref_s)
    print(f"\nPyTorch: range=[{x_enc.min():.3f}, {x_enc.max():.3f}] NaN={x_enc.isnan().any()}")

    exported = export(encode_test, (asr, F0_pred, N_pred, ref_s), strict=False)

    from executorch.exir import to_edge_transform_and_lower
    from executorch.backends.xnnpack.partition.xnnpack_partitioner import XnnpackPartitioner

    print("Lowering with XNNPACK...")
    ...
    pte_path = "exported_pte/debug/encode_xnnpack.pte"
    ...
    if x_enc_et.isnan().any():
        nan_pct = 100 * x_enc_et.isnan().sum().item() / x_enc_et.numel()
        print(f"  ⚠ Has NaNs ({nan_pct:.2f}%)")
    else:
        diff = torch.abs(x_enc - x_enc_et).max().item()
        print(f"  ✓ No NaNs! Max diff: {diff:.6f}")
