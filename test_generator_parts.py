"""
Test generator subcomponents to find which part produces NaNs
Generator has: SourceModuleHnNSF -> Upsample blocks -> Conv -> ISTFT
"""

import torch
from torch import nn
from torch.export import export
from huggingface_hub import hf_hub_download
import os

print("Testing Generator Subcomponents")
print("=" * 80)

from kokoro import KModel

os.makedirs("exported_pte/debug", exist_ok=True)

model = KModel(repo_id="hexgrad/Kokoro-82M", disable_complex=True).eval()

# Generate REAL inputs (abbreviated)
voice_file = hf_hub_download(repo_id="hexgrad/Kokoro-82M", filename="voices/af_bella.pt")
voice_style = torch.load(voice_file, weights_only=True)
if voice_style.dim() == 3:
    voice_style = voice_style.mean(dim=0)
if voice_style.dim() == 2 and voice_style.shape[0] != 1:
    if voice_style.shape[1] == 256:
        voice_style = voice_style[0:1]

phonemes = "həlˈoʊ wˈɝld"
input_ids = list(filter(lambda i: i is not None, map(lambda p: model.vocab.get(p), phonemes)))
TARGET_TOKENS = 16
while len(input_ids) < (TARGET_TOKENS - 2):
    input_ids.append(0)
input_ids = input_ids[:(TARGET_TOKENS - 2)]
input_ids = torch.LongTensor([[0, *input_ids, 0]])

with torch.no_grad():
    input_lengths = torch.tensor(input_ids.shape[-1])
    text_mask = torch.ones((1, input_ids.shape[-1]), dtype=torch.bool)
    bert_dur = model.bert(input_ids, attention_mask=text_mask.int())
    d_en = model.bert_encoder(bert_dur).transpose(-1, -2)
    s = voice_style[:, 128:]
    d = model.predictor.text_encoder(d_en, s, input_lengths, ~text_mask)
    x, _ = model.predictor.lstm(d)
    duration = model.predictor.duration_proj(x)
    duration = torch.sigmoid(duration).sum(axis=-1)
    pred_dur = torch.round(duration).clamp(min=1).long().squeeze()
    device = input_ids.device
    indices = torch.repeat_interleave(torch.arange(input_ids.shape[1], device=device), pred_dur)
    pred_aln_trg = torch.zeros((input_ids.shape[1], indices.shape[0]), device=device)
    pred_aln_trg[indices, torch.arange(indices.shape[0])] = 1
    pred_aln_trg = pred_aln_trg.unsqueeze(0)
    en = d.transpose(-1, -2) @ pred_aln_trg
    x = en.transpose(-1, -2)
    x, _ = model.predictor.shared(x)
    F0 = x.transpose(-1, -2)
    for block in model.predictor.F0:
        F0 = block(F0, s)
    F0_pred = model.predictor.F0_proj(F0).squeeze(1)
    N = x.transpose(-1, -2)
    for block in model.predictor.N:
        N = block(N, s)
    N_pred = model.predictor.N_proj(N).squeeze(1)
    t_en = model.text_encoder(input_ids, input_lengths, ~text_mask)
    asr = t_en @ pred_aln_trg
    ref_s = voice_style[:, :128]

    # Run through encode and decode to get generator input
    F0_conv = model.decoder.F0_conv(F0_pred.unsqueeze(1))
    N_conv = model.decoder.N_conv(N_pred.unsqueeze(1))
    x_cat = torch.cat([asr, F0_conv, N_conv], axis=1)
    x_enc = model.decoder.encode(x_cat, ref_s)
    asr_res = model.decoder.asr_res(asr)

    # Run through decode blocks
    x = x_enc
    res = True
    for block in model.decoder.decode:
        if res:
            x = torch.cat([x, asr_res, F0_conv, N_conv], axis=1)
        x = block(x, ref_s)
        if block.upsample_type != "none":
            res = False
    x_dec = x

print("✓ Generated inputs for generator\n")

from executorch.exir import to_edge_transform_and_lower
from executorch.backends.xnnpack.partition.xnnpack_partitioner import XnnpackPartitioner
from executorch.runtime import Runtime

runtime = Runtime.get()

gen = model.decoder.generator

# ============================================================================
# TEST 1: Just SourceModuleHnNSF (F0 processing)
# ============================================================================
print("=" * 80)
print("TEST 1: SourceModuleHnNSF (F0 -> harmonic source)")
print("=" * 80)

class SourceTest(nn.Module):
    def __init__(self, gen):
        super().__init__()
        self.f0_upsamp = gen.f0_upsamp
        self.m_source = gen.m_source
        self.stft = gen.stft

    def forward(self, f0):
        f0_up = self.f0_upsamp(f0[:, None]).transpose(1, 2)
        har_source, noi_source, uv = self.m_source(f0_up)
        har_source_t = har_source.transpose(1, 2).squeeze(1)
        har_spec, har_phase = self.stft.transform(har_source_t)
        har = torch.cat([har_spec, har_phase], dim=1)
        return har

source_test = SourceTest(gen).eval()

with torch.no_grad():
    har_pt = source_test(F0_pred)
    print(f"PyTorch: shape={har_pt.shape}, range=[{har_pt.min():.3f}, {har_pt.max():.3f}], NaN={har_pt.isnan().any()}")

    exported = export(source_test, (F0_pred,), strict=False)
    har_exp = exported.module()(F0_pred)
    print(f"Exported: range=[{har_exp.min():.3f}, {har_exp.max():.3f}], NaN={har_exp.isnan().any()}")

    try:
        edge_program = to_edge_transform_and_lower(
            exported,
            partitioner=[XnnpackPartitioner()],
        ).to_executorch()

        pte_path = "exported_pte/debug/gen_source.pte"
        with open(pte_path, "wb") as f:
            f.write(edge_program.buffer)

        program = runtime.load_program(pte_path)
        method = program.load_method("forward")
        outputs = method.execute((F0_pred,))
        har_et = outputs[0]

        print(f"ExecuTorch: range=[{har_et.min():.3f}, {har_et.max():.3f}], NaN={har_et.isnan().any()}")

        if har_et.isnan().any():
            print(f"⚠ SourceModuleHnNSF produces NaNs")
        else:
            print(f"✓ SourceModuleHnNSF OK")

    except Exception as e:
        print(f"⚠ Error: {str(e)[:100]}")

# ============================================================================
# TEST 2: Generator without ISTFT (up to conv_post)
# ============================================================================
print("\n" + "=" * 80)
print("TEST 2: Generator upsampling + conv (no ISTFT)")
print("=" * 80)

class GenNoISTFT(nn.Module):
    def __init__(self, gen):
        super().__init__()
        self.f0_upsamp = gen.f0_upsamp
        self.m_source = gen.m_source
        self.stft = gen.stft
        self.noise_convs = gen.noise_convs
        self.noise_res = gen.noise_res
        self.ups = gen.ups
        self.resblocks = gen.resblocks
        self.num_upsamples = gen.num_upsamples
        self.num_kernels = gen.num_kernels
        self.reflection_pad = gen.reflection_pad
        self.conv_post = gen.conv_post

    def forward(self, x, s, f0):
        f0_up = self.f0_upsamp(f0[:, None]).transpose(1, 2)
        har_source, noi_source, uv = self.m_source(f0_up)
        har_source_t = har_source.transpose(1, 2).squeeze(1)
        har_spec, har_phase = self.stft.transform(har_source_t)
        har = torch.cat([har_spec, har_phase], dim=1)

        for i in range(self.num_upsamples):
            x = torch.nn.functional.leaky_relu(x, negative_slope=0.1)
            x_source = self.noise_convs[i](har)
            x_source = self.noise_res[i](x_source, s)
            x = self.ups[i](x)
            if i == self.num_upsamples - 1:
                x = self.reflection_pad(x)
            x = x + x_source
            xs = None
            for j in range(self.num_kernels):
                if xs is None:
                    xs = self.resblocks[i * self.num_kernels + j](x, s)
                else:
                    xs += self.resblocks[i * self.num_kernels + j](x, s)
            x = xs / self.num_kernels

        x = torch.nn.functional.leaky_relu(x)
        x = self.conv_post(x)
        return x

gen_no_istft = GenNoISTFT(gen).eval()

with torch.no_grad():
    spec_pt = gen_no_istft(x_dec, ref_s, F0_pred)
    print(f"PyTorch: shape={spec_pt.shape}, range=[{spec_pt.min():.3f}, {spec_pt.max():.3f}], NaN={spec_pt.isnan().any()}")

    exported = export(gen_no_istft, (x_dec, ref_s, F0_pred), strict=False)
    spec_exp = exported.module()(x_dec, ref_s, F0_pred)
    print(f"Exported: range=[{spec_exp.min():.3f}, {spec_exp.max():.3f}], NaN={spec_exp.isnan().any()}")

    try:
        edge_program = to_edge_transform_and_lower(
            exported,
            partitioner=[XnnpackPartitioner()],
        ).to_executorch()

        pte_path = "exported_pte/debug/gen_no_istft.pte"
        with open(pte_path, "wb") as f:
            f.write(edge_program.buffer)

        program = runtime.load_program(pte_path)
        method = program.load_method("forward")
        outputs = method.execute((x_dec, ref_s, F0_pred))
        spec_et = outputs[0]

        print(f"ExecuTorch: range=[{spec_et.min():.3f}, {spec_et.max():.3f}], NaN={spec_et.isnan().any()}")

        if spec_et.isnan().any():
            nan_pct = 100 * spec_et.isnan().sum().item() / spec_et.numel()
            print(f"⚠ Generator (no ISTFT) produces NaNs ({nan_pct:.2f}%)")
        else:
            print(f"✓ Generator (no ISTFT) OK")

    except Exception as e:
        print(f"⚠ Error: {str(e)[:100]}")

print("\n" + "=" * 80)
print("Check which generator part produces NaNs above")
print("=" * 80)
